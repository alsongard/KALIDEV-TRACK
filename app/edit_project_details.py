from PySide6.QtWidgets import (
    ...
    QVBoxLayout, QMessageBox, QWidget, QLabel, QHBoxLayout, QPushButton, QTextEdit
    )
...
class EditProjectView(QWidget):
    def __init__(self):
        ...
        def set_button_edit_delete():
            editBtn = QPushButton("Edit Project")
            deleteBtn = QPushButton("Delete Button")
        ...
        for item in file_data:
            ...
            editBtn = QPushButton("Edit Project")
            deleteBtn = QPushButton("Delete Project")
            buttonLayout.addWidget(editBtn)
            buttonLayout.addWidget(deleteBtn)
            deleteBtn.clicked.connect(lambda checked, t=title: self.delete_project(t))
    ...
    def delete_project(self, title):
        if title:
            print(f'Deleting project: {self.app_project_folder}/{title}')
            messageBoxResponse = QMessageBox.warning(
                self,
                "warning!",
                f"Do wish to proceed to delete this project: {title}",
            ...
            if messageBoxResponse == QMessageBox.StandardButton.Ok:
                ...
                if os.path.exists(project_remove):
                    ...
                    QMessageBox.information(
                            self,
                    "Information!",
                    f"Path to directory  {project_remove} does not exist",
            ...
            QMessageBox.information(
                self,
                "Information!",
                f"No Project title given: {title}",
                QMessageBox.StandardButton.Ok 
            )
