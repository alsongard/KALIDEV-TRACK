...
class TaskAppView(QMainWindow):
	...
	def __init__(self, username, userID):
		...
		editMenuBarMenue = menuBarObject.addMenu("&Settings")
		...
		addTaskBtn = QPushButton("Add Task For Project")
		...
		fastActionView.addWidget(addTaskBtn)
		...
		if len(geazy) == 0:
			...
			# COMMENTING THIS AS METHOD readFiles already handles directory not exist
			...
			for item  in geazy:
				...
				if project_start_date.date() > todays_date: 
					# if in future
					...
					new_user_data.append([item["project_title"], item["project_priority"], item["project_start_date"], item["project_end_date"], "Future", ''])
					upcomingActivites.append(item["project_title"])
				elif todays_date > project_start_date.date() and project_end_date.date() > todays_date:
					new_user_data.append([item["project_title"], item["project_priority"], item["project_start_date"], item["project_end_date"], "Active", ''])
					activeActivities.append(item["project_title"])
				elif project_end_date.date() < todays_date:
		...
		active_label = QLabel("Active Activities")
		active_label.setStyleSheet("color:#000000")
		active_label.setMargin(10)
		# modifying the font size
		font = active_label.font()
		font.setPointSize(12)
		active_label.setFont(font)
		active_label.setAlignment(Qt.AlignmentFlag.AlignLeft)
		notificationView.addWidget(active_label)
		index = 1
		for item in activeActivities:
			active  = QLabel(f"{index}: {item}")
			notificationView.addWidget(active)
			index += 1


		upComing_label = QLabel("Upcoming Activities")
		upComing_label.setStyleSheet("color:#000000")
		upComing_label.setMargin(10)
		font = upComing_label.font()
		font.setPointSize(12)
		upComing_label.setFont(font)
		upComing_label.setAlignment(Qt.AlignmentFlag.AlignLeft)
		...
		deadlineLabel.setAlignment(Qt.AlignmentFlag.AlignLeft)
...
window = TaskAppView("alsongadizo@gmail.com","95ee5bad432b214b609b0de351d0df09")
window.show()
exit_code = app.exec()
sys.exit(exit_code)
